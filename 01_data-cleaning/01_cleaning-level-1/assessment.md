...
Project GitHub subdirectory URL: https://github.com/prince-parker/d2mr-assessment-prince-parker/tree/main/01_data-cleaning/01_cleaning-level-1
...
This took super long for me to complete but this project demonstrated the usefulness of functions like mutate, select, and those from the stringr family. The most challenging and time-consuming aspect of this project was troubleshooting. Specifically, changing the "car model" column and deleting the column with the wt equation. I am not sure why RStudio was not picking up what I meant in this cases (software often provides suggestions from the environment) but I was able to find some ways to work around these problems. For instance, instead of using select(-wt...), I found a way to generate a table excluding this column through its orientation in the table. 

Example. clean.mtcars <- clean.mtcars[-13]
