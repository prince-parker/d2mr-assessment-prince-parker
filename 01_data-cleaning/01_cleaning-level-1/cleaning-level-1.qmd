...
a. the messy dataset includes an equation to calculate wt.
b. the original dataset replaces column names with abbreviated versions
c. the car model column name is not included in the original data set
d. the data types are different
e. the data types are listed in the messy dataset
...
clean.mtcars <- clean.mtcars %>% rename(mpg=`mpg (Miles/Gallon)`, cyl=cylinders, hp=horsepower, gear=gearbox, carb=carburetors)

colnames(clean.mtcars)[12] <- "model"
...
clean.mtcars <- clean.mtcars %>% relocate(model, mpg, cyl, dsip, hp, drat, wt, qsec, vs, am, gear, carb)
...
There is a length mismatch. Numbers are not rounded. Some values are N/A. 
...
Although I am not a pro at reading the rendered data differences, I prefer this chart because it shows exactly where the data differ. I find render diff and data diff to be easier to understand.
...
+     model = str_replace_all(model, "ToyotA", "Toyota"),
+     model = str_replace_all(model, "T0yota", "Toyota"),
...
The only differences in my dataset and `goal.mtcars` are rounding and the N/A values. I was not sure what to impute the N/A with so I went with the mean...this was incorrect. Also, I rounded to the 2nd decimal place rather than the third. Other than those, my dataset is identical according to render diff and diff data.
